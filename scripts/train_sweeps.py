...
sys.path.append(".")
from scripts.train import train, ESDConfig


def main():
    wandb.init(project="datamodule_sweeps")
    print(wandb.config)
...
if __name__ == "__main__":
    ...
    if parse_args.sweep_file is not None:
        ...
        sweep_id = wandb.sweep(sweep=sweep_config, project="datamodule_sweeps")
        wandb.agent(sweep_id, function=main, count=10)
    else:
        raise FileNotFoundError(f"sweeps.yml not found at {parse_args.sweep_file}")
