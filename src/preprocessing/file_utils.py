...
def process_viirs_filename(filename: str) -> Tuple[str, str]:
    ...
    """
    return (
        (Path(filename).name).replace("DNB_VNP46A1_A", "").replace(".tif", ""),
        "0",
    )
...
def process_s1_filename(filename: str) -> Tuple[str, str]:
    ...
    """
    current = (
        (Path(filename).name).replace("S1A_IW_GRDH_", "").replace(".tif", "").split("_")
    )
...
def process_s2_filename(filename: str) -> Tuple[str, str]:
    ...
    Tuple[str, str]
    """
    current = (Path(filename).name).replace("L2A_", "").replace(".tif", "").split("_")
    return (current[0], current[1].replace("B", ""))
...
def process_landsat_filename(filename: str) -> Tuple[str, str]:
    ...
    """
    current = (
        (Path(filename).name).replace("LC08_L1TP_", "").replace(".tif", "").split("_")
    )
    return (current[0], current[1].replace("B", ""))
...
def load_satellite(
    ...
    sat_data_stack, sat_filenames = stack_satellite_data(
        sat_data, sat_files, satellite_type
    )
...
def stack_satellite_data(
    ...
    for key, group in groupby(
        zip(date_bands, sat_data, file_names), key=lambda x: x[0][0]
    ):
