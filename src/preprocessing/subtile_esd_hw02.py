...
class Subtile:
    ...
    def save(self, dir: str | os.PathLike):
        ...
        np.savez(
            dir
            / "subtiles"
            / f"{self.tile_metadata.parent_tile_id}_{self.tile_metadata.x_gt}_{self.tile_metadata.y_gt}.npz",
        ...
        self.tile_metadata.saveJSON(
            dir
            / "metadata"
            / f"{self.tile_metadata.parent_tile_id}_{self.tile_metadata.x_gt}_{self.tile_metadata.y_gt}.json"
        )
...
def get_tile_ground_truth(
    ...
    Outputs:
        gt_child: np.ndarray
            ...
            time and bands are 1 and bands is 1 for ground truth.
    """
    ...
    start_index = (size_gt[0] * x_gt, size_gt[1] * y_gt)
    end_index = (size_gt[0] * (x_gt + 1), size_gt[1] * (y_gt + 1))

    # check to make sure the slice is within the bounds of
    # the ground truth parent image
    if start_index[0] > gt_parent.shape[2] or start_index[1] > gt_parent.shape[3]:
        raise IndexError(f"Start index {start_index} out of range")

    if end_index[0] > gt_parent.shape[2] or end_index[1] > gt_parent.shape[3]:
        raise IndexError(f"End index {end_index} out of range")

    # return the slice of the ground truth parent image
    return np.copy(
        gt_parent[:, :, start_index[0] : end_index[0], start_index[1] : end_index[1]]
    )
...
def get_tile_satellite(
    ...
    outputs:
        sat_child: np.ndarray
            ...
            where time and bands are 1 and bands is 1 for ground truth.
    """
    ...
    # calculate the start and end indices for the slice based
    # on the size_gt and the x and y coordinates
    start_index = (scale_factor * size_gt[0] * x_sat, scale_factor * size_gt[1] * y_sat)
    end_index = (
        scale_factor * size_gt[0] * (x_sat + 1),
        scale_factor * size_gt[1] * (y_sat + 1),
    )

    if start_index[0] > sat_parent.shape[2] or start_index[1] > sat_parent.shape[3]:
        raise IndexError(f"Start index {start_index} out of range")

    if end_index[0] > sat_parent.shape[2] or end_index[1] > sat_parent.shape[3]:
        raise IndexError(f"End index {end_index} out of range")

    # return the slice of the satellite parent image
    return np.copy(
        sat_parent[:, :, start_index[0] : end_index[0], start_index[1] : end_index[1]]
    )
...
def grid_slice(
    ...
    if "sentinel1" in satellite_stack:
        sat_stack = satellite_stack["sentinel1"]
    elif "sentinel2" in satellite_stack:
        sat_stack = satellite_stack["sentinel2"]
    elif "landsat" in satellite_stack:
        sat_stack = satellite_stack["landsat"]
    elif "viirs" in satellite_stack:
        sat_stack = satellite_stack["viirs"]

    scale_factor = sat_stack.shape[2] // gt_stack.shape[2]
    ...
    num_tiles_x = gt_stack.shape[3] // tile_size_gt

    # Calculate the number of tiles needed in the y dimension
    num_tiles_y = gt_stack.shape[2] // tile_size_gt
    ...
    for x in range(num_tiles_x):
        for y in range(num_tiles_y):
            ...
            for sat_type, data in satellite_stack.items():
                # gt is a special case
                if sat_type == "gt":
                    # get the current slice for ground truth using get_tile_ground_truth
                    slice = get_tile_ground_truth(data, x, y, tile_size_gt)
                else:
                    # otherwise use the get_tile_satellite function
                    slice = get_tile_satellite(data, x, y, tile_size_gt, scale_factor)

                tile_stack[sat_type] = slice
            ...
            subtile = Subtile(tile_stack, tile_metadata)
...
def restitch(
    ...
    """
    # Initialize stitched image and TileMetadata list
    stitched_image = list()
    tile_metadata_list = list()

    # Iterate through the entire ranges of x and y
    for x in range(range_x[0], range_x[1]):
        # Initialize the stitch along the width of the image and the metadata for those tiles
        width_stitch = list()
        metadata = list()

        for y in range(range_y[0], range_y[1]):
            # Load the Subtile object
            subtile = Subtile().load(f"{dir}/{tile_id}_{x}_{y}.npz")

            # Obtain the correct satellite image and concatenate it to width_stitch
            data = subtile.satellite_stack[satellite_type]
            width_stitch = (
                data
                if len(width_stitch) == 0
                else np.concatenate((width_stitch, data), axis=3)
            )

            # Append the corresponding TileMetadata
            metadata.append(subtile.tile_metadata)

        # Stitch the image with the width_stitch and append the metadata list
        stitched_image = (
            width_stitch
            if len(stitched_image) == 0
            else np.concatenate((stitched_image, width_stitch), axis=2)
        )
        tile_metadata_list.append(metadata)

    return stitched_image, tile_metadata_list
